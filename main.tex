...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % 中文封面内容
  \title{室内场景的点云协同分割}%一般情况下扉页和封皮、书脊共用一个标题文本，可以不用定义\spinetitle（仅硕博有用）, \covertitle（本硕博均有用）和\encovertitle（仅本科有用）。特殊情况见下。
  ...
  \author{林\ 彦\ 廷}
  \depart{系}%系别，硕博请用系代号，本科请用全称如
  %\depart{数理化和信息工程系}
  \major{电子与通信工程专业}%专业，硕博请用全称，本科不需要
  \advisor{陈雪锦\ 副教授}
  %\coadvisor{冯晨珠\ 教授}%第二导师，没有请注释掉
  \studentid{SA14006106}%For bachelor only
  \submitdate{二〇一七年五月}

  % 英文封面内容
  \entitle{Co-Segmentation of 3D Point Clouds for Indoor Scene}
  \enauthor{Yanting Lin}
  \enmajor{Electronics and Communication Engineering}
  \enadvisor{A/Prof. Xuejin Chen}
...
\mainmatter
  ...
  \include{chapter/hieseg}
  \include{chapter/feature}
  \include{chapter/coana}
